...
class PasswordGenerator(Tk):
    ...
    def collect_password(self, length, strength):
        ...
        try:
            length = int(length)
            if length > 1000 or length < 5:
                msg.showwarning(title = "WARNING",message = "password length must be less than 1000 and greater than 4")
            elif length <= 1000 or length >= 5:
